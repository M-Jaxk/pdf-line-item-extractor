...
### Part A — extraction service

- [x] Multipart PDF upload at `POST /api/extract`, with signature, empty-file, and 20 MB validation.
- [x] Page-level PDF text extraction with page number and reconstructed source-line evidence.
- [x] JSON output with extracted items, separate refusals, warnings, and read statistics.
- [x] Conservative refusal for ambiguous numbers, uncertain columns, missing amounts, and contradictory line totals.
- [x] Surface conflicting logistics counts with both source notes rather than selecting one.
- [x] Structured document refusals for image-only, malformed, password-protected, and over-page-limit PDFs.
- [x] Fault isolation for unreadable pages and rows.
- [x] Unit, API, evidence, presentation, and provided-sample regression tests.

### Part B — review page

- [x] Responsive file browser and drag-and-drop PDF upload.
- [x] File selection, upload progress, validation, and retryable error states.
- [x] Actual API/network failure reasons displayed to the user.
- [x] Extracted line items with expandable source evidence.
- [x] Refusals displayed in plain language with page and quoted source.
